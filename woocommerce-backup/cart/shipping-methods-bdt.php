	...
	foreach ( $available_methods as $method ) {
		...
		if ( $method->cost > 0 ) {
			if ( $woocommerce->cart->tax_display_cart == 'excl' ) {
				$method->full_label .= ': ' . woocommerce_price( $method->cost ) . sbw_bdt_wrap( sbw_usd_to_bdt( $method->cost ) );
				...
				$method->full_label .= ': ' . woocommerce_price( $method->cost + $method->get_shipping_tax() ) . sbw_bdt_wrap( sbw_usd_to_bdt( $method->cost ) );
				if ( $method->get_shipping_tax() > 0 && ! $woocommerce->cart->prices_include_tax ) {
